...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const toRegisterBtn = document.querySelector(
    ".to-register"
  ) as HTMLButtonElement;
  if (toRegisterBtn) {
...
async function handleRegister() {
  ...
  const entreprise = (document.getElementById("entreprise") as HTMLInputElement)
    ?.value;
  const adress = (document.getElementById("adresse") as HTMLInputElement)
    ?.value;
  ...
  try {
    ...
    if (
      ...
      showError(
        "Erreur de communication avec le serveur. Vérifiez la configuration.",
        "register-error"
      );
    } else {
      showError(
